...
- Do not invoke `superpowers:subagent-driven-development` or `superpowers:dispatching-parallel-agents`.
- Do not call multi-agent tools for implementation, review, exploration, validation, or planning.
- If another workflow recommends subagents, use the inline/current-session alternative instead.
- Self-review every diff inline. Use a separate reviewer only when the user explicitly asks for it.
